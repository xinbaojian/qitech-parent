...
import com.qitech.qitechoauth2.enums.GrantTypeEnum;
import com.qitech.qitechoauth2.constant.SecurityConstant;
import com.qitech.qitechoauth2.security.validate.userdetails.UserDetailsServiceHolder;
...
public class CustomTokenController {
    ...
    private final @NonNull UserDetailsServiceHolder userDetailsServiceHolder;
    ...
    public HttpEntity<?> auth(HttpServletRequest request, @PathVariable String type) {
        ...
        // 判断是否是我们自定义的授权类型
        if (!GrantTypeEnum.hasGrantType(type)) {
        ...
        if (header == null || !header.toLowerCase().startsWith(SecurityConstant.BASIC_TYPE)) {
        ...
        // 4. 获取当前用户信息
        UserDetails userDetails = userDetailsServiceHolder.findUserDetailsService(type).loadUserByUsername(request.getParameter(type));
        // 5. 构建用户授权令牌
